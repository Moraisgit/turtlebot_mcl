...
def allParticles_position():
    ...
    plt.scatter(particles_x_pixels, particles_y_pixels, color='blue', label='Particles positions',s=6,alpha=0.6)
...
def initialization():
    ...
    map, width, height = read_pgm("/home/morais/Downloads/corredor_mais_sala.pgm", byteorder='<')

    with open("/home/morais/Downloads/corredor_mais_sala.yaml", 'r') as file:
   ...
   # ax.legend()
    ...
    line_amcl = ax2.plot([], [], color='green', linestyle='-', linewidth=2, label='Robot's position')
    scatter_particles2 = ax2.scatter([], [], color='purple', s=5, label='Particles positions) """
...
def save_data(signal, frame):
    ...
    data.to_csv('~/amcl_csv/amcl_results_{timestamp}.csv', index=False)
    data1.to_csv('~/particle_csv/particles_results_{timestamp}.csv', index=False)

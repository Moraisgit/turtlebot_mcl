...
"""
        ...
        ####  Graph Node 2 Microsimulator   ((RMSE and trajectories )     ####
        ####          Sistemas Autónomos                                  ####
...
def initialization():
    ...
    scatter_amcl = ax.scatter([], [], color='red', s=5, label='Robot trajectory (/robotpose)')
...
def main():
    plt.ion()
    rospy.init_node('MS_Graphs_2', anonymous=True)
    rospy.Subscriber("/robotpose", MarkerArray, amcl_callback)
    ...
    while not rospy.is_shutdown():
        get_position()
        with open(rospy.get_param("~csv_path", default="/home/morais/turtle_ws/src/turtlebot_mcl/csv/micro_simulator/rmse_data.csv"), 'w', newline='') as file:  # Change the path as needed
